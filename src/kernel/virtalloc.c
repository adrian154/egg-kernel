...
// The physical allocator (physalloc.c) manages physical pages, but we also need a way to retrive free virtual addresses.
// That's handled in part by the virtual allocator, this code.
// This allocator manages the virtual address range reserved for the kernel.

// Address of the next free virtual address.
// Mitigates the need to search the kernel address space from the start for a free space.
void *nextFree;
